...
def dms_esm(fname, model_locations):
    ...
    df_esm = predict_esm(wt_seq, model_locations, mutation_col="variant", offset_idx=1)
    summed_log_probs = []
    for idx, row in df_esm.iterrows():
        summed_log_probs.append(
            sum([row[model_location] for model_location in model_locations])
        )
    ...
    esm_vote, esm_vote_component = (
        [],
        {name: [] for _, names in mutations_model_names.items() for name in names},
    )
