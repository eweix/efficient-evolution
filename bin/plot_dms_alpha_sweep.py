...
if __name__ == "__main__":
    settings = [
        ...
        ["dms_p53", "DMS_null_etoposide", 1.0],
        ["dms_pafa", "DMS_kcat_km", 2300000.0],
    ]
    ...
    for alpha in [
        ...
        for namespace, dms_name, cutoff in settings:
            ...
            df_setting = df_lm[
                (df_lm["namespace"] == namespace) & (df_lm["dms_name"] == dms_name)
            ]

            n_positive = len(df_setting[(df_setting["fitness"] > cutoff)])
            n_total = len(df_setting)
            if n_total == 0:
                continue
            lm_guided_pct = n_positive / n_total * 100.0
            if alpha == 0.5 and namespace == "dms_infa":
                print("Protein infa")
                print(f"\talpha = {alpha}, {n_positive} / {n_total} = {lm_guided_pct}%")
    ...
    for namespace, dms_name, cutoff in settings:
        ...
        background_pct = n_positive / n_total * 100.0
        print(
            f"Protein {namespace}, background: {n_positive} / {n_total} = {background_pct}%"
        )
        ...
        for k in range(max_models):
            n_positive = len(
                df_setting[
                    (df_setting["n_models"] > k) & (df_setting["fitness"] > cutoff)
                ]
            )
            n_total = len(df_setting[(df_setting["n_models"] > k)])
            if n_total < 5:
                continue
            lm_guided_pct = n_positive / n_total * 100.0
    ...
    df = pd.DataFrame(topline_data, columns=["namespace", "pct_type", "pct"])
    plt.figure()
    sns.barplot(
        data=df,
        x="namespace",
    ...
    for val_name, palette in zip(val_names, palettes):
        ...
        sns.barplot(data=df, x="namespace", y=val_name, hue="alpha", palette=palette)
