...
if __name__ == "__main__":
    ...
    for namespace, dms_name, cutoff, sample_size in settings:
        ...
        for column in df.columns:
            ...
            finite_idx = np.logical_and(
                np.isfinite(prediction), np.isfinite(experiment)
            )
    ...
    for metric in [
        ...
        to_highlight = {model for idx, model in enumerate(top_models["model"])}
        colors = [
            "#e8bcf9",
            "#fdda0d",
            "#add8e6",
